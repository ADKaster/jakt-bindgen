...
llvm::Optional<clang::QualType> JaktGenerator::getTemplateParameterIfMatches(clang::QualType const& type, llvm::StringRef template_name, unsigned int index) const
{
...
void JaktGenerator::printClassDeclaration(clang::CXXRecordDecl const* class_definition)
{
    ...
    for (auto const& base : class_definition->bases()) {
        if (base.isVirtual())
            throw std::invalid_argument("Virtual base param");
        if (base.getAccessSpecifier() != clang::AccessSpecifier::AS_public)
            throw std::invalid_argument("ERROR: Don't know how to handle non-public bases");
        ...
        if (!base_record)
            throw std::invalid_argument("ERROR: Base class unusable");
...
void JaktGenerator::printClassMethods(clang::CXXRecordDecl const* class_definition)
{
    ...
    for_each_method([&](clang::CXXMethodDecl const* method) {
        ...
        if (isErrorOr(method->getReturnType())) {
            m_out << "throws ";
            flags |= QualTypePrintFlags::PF_InThrowsyFunction;
        }
        m_out << "-> ";
        if (is_constructor)
...
std::string JaktGenerator::rewriteQualTypeToJaktType(clang::QualType const& base_type, QualTypePrintFlags flags)
{
    ...
    if (has_flag(flags, QualTypePrintFlags::PF_InThrowsyFunction) && has_flag(flags, QualTypePrintFlags::PF_IsReturnType)) {
    ...
    if (auto inner_type = getTemplateParameterIfMatches(type, "AK::NonnullRefPtr"); inner_type.has_value())
        return rewriteQualTypeToJaktType(inner_type.value(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction);
    ...
    if (auto inner_type = getTemplateParameterIfMatches(type, "AK::DynamicArray"); inner_type.has_value())
        return std::string("[") + rewriteQualTypeToJaktType(inner_type.value(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction) + "]";
    ...
    if (auto key_type = getTemplateParameterIfMatches(type, "Jakt::Dictionary"); key_type.has_value()) {
        ...
        return std::string("[")
            + rewriteQualTypeToJaktType(key_type.value(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction)
            + std::string(":")
            + rewriteQualTypeToJaktType(value_type.value(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction)
            + "]";
    }

    if (auto inner_type = getTemplateParameterIfMatches(type, "AK::WeakPtr"); inner_type.has_value()) {
        return std::string("weak ")
            + rewriteQualTypeToJaktType(inner_type.value(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction)
            + "?";
    }

    if (auto inner_type = getTemplateParameterIfMatches(type, "AK::Function"); inner_type.has_value()) {
        std::string jakt_type = "function(";
        auto function_type = inner_type.value()->getAs<clang::FunctionProtoType>();
        if (!function_type)
        ...
        if (isErrorOr(function_type->getReturnType())) {
            jakt_type += " throws";
            print_flags |= QualTypePrintFlags::PF_InThrowsyFunction;
        }
    ...
    if (auto const* reference_type = type->getAs<clang::ReferenceType>()) {
        ...
        return prefix + rewriteQualTypeToJaktType(reference_type->getPointeeType(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction);
    }

    if (auto const* pointer_type = type->getAs<clang::PointerType>())
        return std::string("raw ") + rewriteQualTypeToJaktType(pointer_type->getPointeeType(), flags & ~QualTypePrintFlags::PF_InThrowsyFunction);
    ...
    if (auto const* builtin_type = type->getAs<clang::BuiltinType>()) {
        ...
        default:
            throw std::invalid_argument("bruh I can't convert these fancy types to jakt");
        }
    }
    ...
    throw std::invalid_argument("what the fuck is this?");
}
